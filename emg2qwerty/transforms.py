...
@dataclass
class DownsampleNonInteger:
    """
    Downsamples the input signal along the time dimension by a non-integer factor using interpolation.

    Args:
        factor (float): The downsampling factor. The original sampling rate is 2000 Hz,
                        a factor of 1.5 reduces the effective sampling rate to about 1333 Hz.
        mode (str): The interpolation mode. Typically 'linear'.
    """
    factor: float = 1.5
    mode: str = 'linear'
    ...
    def __call__(self, data: torch.Tensor) -> torch.Tensor:
        T, N, C = data.shape
        new_T = max(1, int(T / self.factor))

        data_perm = data.permute(1, 2, 0)
        data_resized = F.interpolate(data_perm, size=new_T, mode=self.mode, align_corners=False)

        return data_resized.permute(2, 0, 1)


@dataclass
class Downsample:
    """
    Downsamples the input signal along the time dimension by taking every nth sample.
    
    Args:
        factor (int): The downsampling factor. The original sampling rate is 2000 Hz,
                      a factor of 2 reduces the effective sampling rate to about 1000 Hz.
    """
    factor: int = 2
...
class SliceChannels:
    """
    Slices the left and right hand channels of a tensor with shape (T, 2, 16),
    keeping only the specified indices. 

    Args:
        indices (Sequence[int]): A list of channel indices to keep.
    """
    indices: Sequence[int]

    def __call__(self, tensor: torch.Tensor) -> torch.Tensor:
        left, right = tensor.unbind(dim=1)
        left_sliced = left[:, self.indices]
        right_sliced = right[:, self.indices]
        return torch.stack([left_sliced, right_sliced], dim=1)
...
class RandomAmplitudeScaling:
    """
    Randomly scales the amplitude of the EMG signals by a factor sampled uniformly
    from the provided range.

    Args:
        scale_range (Sequence[float]): A tuple (min_scale, max_scale) representing
            the range from which to sample the scaling factor.
        per_channel (bool): If True, a different scaling factor is applied to each channel.
            Otherwise, a single factor is applied to all values.
    """
    ...
    def __call__(self, data: torch.Tensor) -> torch.Tensor:
        if self.per_channel:
            scale_factors = torch.tensor(
                np.random.uniform(self.scale_range[0], self.scale_range[1], size=(data.shape[-1],)),
                dtype=data.dtype,
                device=data.device
